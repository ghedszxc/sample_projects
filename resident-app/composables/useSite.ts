export default function useSite() {
  ...
  async function getAllSites(
    { page = 1, organization = "", search = "" } = {} as {
      page?: number;
      organization?: string;
      search?: string;
    },
  ) {
    const { error, data } = await useLocalFetch("/api/sites/v1/all", {
      method: "GET",
      query: { page, organization, search },
    });
  ...
  async function updateSiteStatusById(_site: TUpdateSiteStatusOptions) {
    const { error, data } = await useLocalFetch(
      ...
      {
        method: "PUT",
        body: JSON.stringify({ status: _site.status }),
      },
    );
