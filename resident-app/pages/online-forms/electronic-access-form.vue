<template>
  <v-row no-gutters justify="center" class="px-5">
    ...
    <FormsAddElectronicAccessForm @on-submit="onSubmit" />
  </v-row>
</template>

<script setup lang="ts">
definePageMeta({
  key: "electronic-access-form",
  layout: "resident-bottom-navigation",
  middleware: "secure",
});

const { addItem, updateItem } = useElectronicAccessCard();
...
async function onSubmit(data: TElectronicAccessCard, queryAction: string) {
  if (queryAction === "edit") {
