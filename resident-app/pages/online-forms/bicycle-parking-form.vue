<template>
  <v-row no-gutters justify="center" class="px-5">
    ...
    <FormsAddBicycleParkingForm @on-submit="onSubmit" />
  </v-row>
</template>

<script setup lang="ts">
definePageMeta({
  key: "bicycle-parking-form",
  layout: "resident-bottom-navigation",
  middleware: "secure",
});

const { addItem, updateItem } = useBicycle();
...
async function onSubmit(data: TBicycleRecord, queryAction: string) {
  if (queryAction === "edit") {
