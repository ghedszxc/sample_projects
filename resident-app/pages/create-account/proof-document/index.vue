<template>
  <v-row no-gutters class="px-6">
    ...
    <!-- attach pdf/doc file to upload -->
    <v-col cols="12" class="pa-4 mt-6">
      <v-file-input
        class="file-input"
        :base-color="baseColor"
        :bg-color="bgColor"
        label="Tenancy Agreement/Tenancy Agreement Stamp"
        prepend-icon=""
        v-model="files"
        :model-value="null"
        accept=".pdf"
...
function goNext() {
  ...
  const { tenantDocument } = resident.value;
  if (tenantDocument.length > 0) {
    localStorage.setItem("resident", JSON.stringify(resident.value));
    useRouter().push({ name: "create-account-confirm-details" });
  } else {
    setSnackbar({
      modal: true,
      text: "Please upload tenant document",
      type: "error",
    });
  }
}
...
onMounted(() => {
  ...
  if (values) {
    ...
    if (resident.value.status === "resubmit") {
      files.value = resident.value.tenantDocument;
    }
  } else {
    useRouter().push({ name: "create-account-info" });
  }

  // resident.value.tenantDocument = [];
});
  ...
  for (let i = 0; i < newFiles.length; i++) {
    ...
    xhr.onload = () => {
      if (xhr.status === 200) {
        ...
        if (response?.id) {
          resident.value.tenantDocument.push({
            _id: response.id,
...
function removeFile(id: string) {
  resident.value.tenantDocument = resident.value.tenantDocument.filter(
    (item) => item._id !== id,
  );
}
