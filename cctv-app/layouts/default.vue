<template>
  <v-app :theme="theme">
    ...
    <v-navigation-drawer v-model="drawer" color="navigationDrawer">
      <v-row no-gutters justify="center" class="mt-4 text-center">
            ...
            <template v-if="!service?.sublist">
              ...
              >
                ...
                <!-- <template v-slot:append>
                  ...
                  unreadNotifications[
                    Object.keys(unreadNotifications).find((key) =>
                      new RegExp(`^${service.name.replace(/s$/, '')}(s?)$`).test(
                        key,
                      ),
                    ) as any
                  ]
            ...
            <template v-else>
              <v-list-group v-if="service.gate">
                ...
                >
                  ...
                  unreadNotifications[
                    Object.keys(unreadNotifications).find((key) =>
                      new RegExp(`^${item.name.replace(/s$/, '')}(s?)$`).test(
                        key,
                      ),
                    ) as any
                  ]
    ...
    <v-footer height="50" class="px-0 text-caption" app>
      <v-row
        class="d-flex px-12 align-items-center w-100 justify-space-between"
        align="center"
        align-content="center"
        style="flex-wrap: nowrap"
      >
        <span class="text-truncate">© 2024 by Seven 365 Pte Ltd V1.0</span>
...
const siteId = computed(() => mySite.value);
...
// const { visitorSocket, organizationSocket, feedbackSocket, incidentSocket } =
//   useSocket();
...
async function getHeaderTitle() {
  ...
  name.value = `${orgOrSpmName} ${sptOrSite ? " | " + sptOrSite : ""}`;
}
