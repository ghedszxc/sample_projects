				...
				<v-form v-model="isCctvValid" @submit.prevent="submit()">
					<v-row no-gutters class="pa-8" justify="center">
						...
						<v-col cols="12" class="mb-4">
							<v-text-field v-model="cctv.location" density="comfortable" :rules="[requiredInput]"
...
async function submit() {
	try {
		const result = await updateCCTV({
			_id: cctv.value._id,
			location: cctv.value.location,
		});
		setSnackbar({ text: "Successfully update camera location", modal: true, type: "success" });
		goBack();
	} catch (error) {
