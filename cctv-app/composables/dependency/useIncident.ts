...
export default function useIncident() {
  ...
  async function getAllIncidents(
    ...
    const { error, data } = await useLocalFetch(
      `${incidentApiUrl}/all-incidents`,
      {
        method: "GET",
        query: { search, organization, site },
      },
    );

    // const { error, data } = await useLocalFetchUtil<IIncident>(
    //   `${incidentApiUrl}/all-incidents`,
    //   { method: "GET", query: { search, organization, site } },
    // );
    // isLoaded.value = true;
    return data;
  }
