...
export default function useSocket() {
  ...
  const eventSocket = io(`${config.public.API_REAL_TIME}event-events`, {
    autoConnect: false,
  });
  const authorizationSocket = io(
    `${config.public.API_REAL_TIME}authorization-events`,
    {
      autoConnect: false,
    },
  );
  const facilitySocket = io(`${config.public.API_REAL_TIME}facility-events`, {
    autoConnect: false,
  });
  return {
    visitorSocket,
