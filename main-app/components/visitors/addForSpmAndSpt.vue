          ...
          <v-col cols="12" class="mb-4" v-if="visitor.type == 'guest'">
            <v-form ref="guestForm">
              ...
              <v-col cols="12" v-if="addVisitorSteps === 1">
                <v-row class="pb-8" justify="flex-start">
                  <v-switch
                    v-model="isTemporary"
                    label="Temporary Register"
                    color="blue-darken-3"
                    density="compact"
                    hide-details
                    inset
              ...
              <v-row v-if="addVisitorSteps == 1">
                <v-col class="d-flex ga-12" cols="12" v-if="!isTemporary">
                  <v-text-field
              ...
              <v-row v-if="addVisitorSteps === 2" class="mt-5">
                <v-col cols="12">
                  <v-radio-group v-model="visitor.optionAccessCard" inline>
                    <v-radio
                      label="Physical Pass"
                      value="Physical Pass"
                      color="primary"
                    ...
                    ></v-radio>
                    <v-radio
                      label="Non Physical Pass"
                      value="Non Physical Pass"
                      color="primary"
                ...
                <v-col cols="12">
                  <v-card
                    class="border pa-4"
                    v-if="
                      visitor.optionAccessCard == 'Physical Pass' ||
                      visitor.optionAccessCard == 'Non Physical Pass'
                    "
                  >
                  ...
                  <v-text-field
                    v-else-if="
                      visitor.optionAccessCard == 'Physical NFC Card' ||
                      visitor.optionAccessCard == 'QR Code Pass'
                    "
                    density="compact"
                    hide-details
                    :rules="[requiredInput]"
              ...
              <v-col cols="12">
                <v-btn
                  v-if="addVisitorSteps === 1 && !isTemporary"
                  block
                  color="blue-darken-3"
                  size="small"
                  variant="flat"
                  ...
                  <v-row no-gutters>
                    ...
                    <v-col cols="12" class="mb-6" v-if="addVisitorSteps <= 3">
                      ...
                      <v-row v-if="addVisitorSteps === 2" class="mt-5">
                        <v-col cols="12">
                          <v-radio-group
                            v-model="visitor.optionAccessCard"
                            inline
                          >
                            <v-radio
                              label="Physical Pass"
                        ...
                        <v-col cols="12">
                          ...
                          >
                            ...
                            <div class="font-weight-bold mt-4 mb-2">Keys</div>
            ...
            <v-form ref="deliveryForm">
              <v-row v-if="addVisitorSteps === 1">
                ...
                <v-col cols="12">
                  <v-file-input
                    label="Upload/Scan"
                    hint="Scan/Upload NRIC/Passport/ID No. Only"
                    prepend-icon="mdi-camera"
                    variant="solo-filled"
                    density="compact"
                    persistent-hint
                    @update:modelValue="onCaptureNric($event)"
                  ></v-file-input>
                </v-col>
...
const onHideOcrScanner = (value: any) => {
  isNricScanning.value = false;
  visitor.value.nric = value.nric;
  visitor.value.firstName = value.name;
};
...
const onCaptureNric = async () => {
  try {
    ...
    const getFile = await fetch(imageData);
    const blob = await getFile.blob();
    const uploadFile = new File([blob], "nric_card", { type: "image/png" });
    // const uploadFile = Array.isArray(file) ? file[0] : file;
    const scanNric = await getScannedNricInfo(uploadFile);
    const { result } = scanNric!;

    visitor.value.nric = result.nric;
    visitor.value.firstName = result.name;
    setSnackbar({
      text: "Nric successfully detected.",
      type: "success",
      modal: true,
    });
    closeNricScan();
...
watch(
  ...
  (newValue, oldValue) => {
    if (!newValue.length) {
      const findBlock = oldValue.filter(
        (find) => find._id == visitor.value.block,
      );
      blocks.value = findBlock;
    }
  },
  { deep: true },
);
...
async function submitTemporary() {
  ...
  try {
    ...
    if (visitor.value.firstName && visitor.value.lastName) {
      ...
      visitor.value.name = visitor.value?.firstName;
    } else if (!visitor.value.firstName && visitor.value.lastName) {
      visitor.value.name = visitor.value?.lastName;
    } else if (!visitor.value.firstName && !visitor.value.lastName) {
      visitor.value.name = "";
    }
    ...
    addVisitorSteps.value = visitor.value.type === "contractor" ? 4 : 3;
...
async function onAddVisitorFetch(dataArgument: any | undefined = undefined) {
  try {
    ...
    let data = visitor.value;
    if (dataArgument) {
      dataArgument.name = visitor.value.name;
      ...
      if (
        ...
        if (
          ...
          dataArgument.type === "delivery" ||
          dataArgument.type === "walk-in" ||
          dataArgument.type === "pick-up" ||
          dataArgument.type === "drop-off"
        ) {
