<template>
  <v-dialog max-width="1500" v-model="isShowDialog" persistent>
    <v-card class="rounded-xl" min-height="600px">
      <v-toolbar>
        <span class="text-h6 d-none d-md-inline-flex ml-8">
        ...
        <v-row align="center" justify="end" class="px-2">
          ...
          <v-col cols="12" md="4">
            <v-btn
              ...
              color="primary"
              width="100%"
              @click="emit('onShowInviteServiceProviderDialog')"
            >
        ...
        <v-btn
          icon="mdi-close"
          @click="emit('onHideDataListingDialog')"
        ></v-btn>
      </v-toolbar>
      <v-divider></v-divider>

      <v-card-text class="py-0 px-1 overflow-auto" style="max-height: 800px">
        <g-card :loading="isServiceProviderLoading" :elevation="0">
          <v-data-iterator
          ...
          >
                        ...
                        <v-row no-gutters>
                          ...
                          >
                            ...
                            >
                              ...
                              >
                                ...
                                {{ item.raw.name || "N/A" }}</span
                              >
                            </v-row>
                          </v-col>

                          <v-col cols="12" md="3" class="text mb-2 mb-sm-0">
                            <v-row
                              no-gutters
                              class="fill-height text-truncate"
                              align-content="center"
                            ...
                            >
                              ...
                              >
                                <v-icon class="ml-1 mr-1 mr-md-6">
                                  mdi-phone </v-icon
                                >{{ item.raw.contactNumber || "N/A" }}</span
                              >
                            </v-row>
                          </v-col>
                          ...
                          >
                            ...
                            >
                              ...
                              <span
                                v-if="typeof item.raw.service === 'string'"
                                :style="
                                  $vuetify.display.smAndDown
                                    ? 'font-size: .8rem;'
                                    : ''
                                ...
                                >{{ item.raw.service || "N/A" }}
                              </span>
                              <span v-else>
                                {{
                                  (item.raw.service && item.raw.service[0]) ||
                                  "N/A"
                                }}
                              </span>
                            </v-row>
                            ...
                            >
                              <v-menu>
                                <template v-slot:activator="{ props }">
                                  <v-icon density="compact" v-bind="props" class="rounded-xl pa-4 border-md">
                                    mdi-dots-vertical
                                  </v-icon>
                                </template>
                                <v-list>
                                  <v-list-item
                                    :disabled="
                                      ...
                                      /^inactive$/i.test(item.raw.status || '')
                                    "
                                    @click="onInviteServiceProvider(item.raw)"
                                  >
                                    <template #title>
            ...
            <template #no-data>
              <v-row no-gutters justify="center" class="pa-16">
                ...
                <v-col cols="12">
                  <v-row no-gutters justify="center">
                    <span class="font-weight-bold"> No service provider. </span>
                  </v-row>
                </v-col>
              </v-row>
            </template>
...
const props = defineProps<{
  isShowDataListingDialog: boolean;
}>();
const emit = defineEmits<{
  (event: "onHideDataListingDialog"): void;
  (event: "onShowInviteServiceProviderDialog"): void;
}>();
...
const header = computed<{ md: string; title: string; align: string }[]>(() => {
  return [
    {
      md: "3",
      title: "Name",
...
const isShowDialog = computed(() => props.isShowDataListingDialog);
...
async function onFilter(keyword: string) {
  setServiceProviders({
    service: keyword || selectFilter.value,
  });
}
...
async function onInviteServiceProvider(value: TServiceProvider) {
  try {
    await createSiteCollab({
      serviceProvider: value._id || "",
      site: siteId.value || String(currentUser.value.site),
      status: "pending",
    });
