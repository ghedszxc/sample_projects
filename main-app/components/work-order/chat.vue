...
const idWorkOrder = computed(() => useRoute().params.id as string);
const fileUploaded = ref([]);
const showScrollButton = ref(false);
const chatContent = ref(null);

onMounted(async () => {
  await setComments(idWorkOrder.value, "workOrder");
});

async function sendMessage() {
  if (comment.value.comment) {
    comment.value._id = idWorkOrder.value;
    comment.value.createdBy = currentUser.value._id;
    comment.value.feedback = idWorkOrder.value;
    ...
    // );
    comment.value.workOrder = idWorkOrder.value;
