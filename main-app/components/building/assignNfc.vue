    ...
    >
      ...
      <v-card-text>
        <v-list>
          ...
          <template
            v-for="(data, key) in dummyUsers.filter((find) =>
              search
          ...
          >
            ...
            >
              <v-icon
                size="small"
                class="mr-2"
                :color="selectedUser.name == data.name ? 'green' : 'grey'"
              >
                {{
                  `mdi-${selectedUser.name == data.name ? "radiobox-marked" : "radiobox-blank"}`
                }}
              </v-icon>
              {{ data.name }}
            </v-list-item>
...
const { accessCards } = useEntryPass();
const { theme, setSnackbar } = useLocal();

const search = ref("");
const selectedUser = ref({ name: "", category: "" });
const dummyUsers = ref([
  { name: "John Doe", category: "Tenant" },
  { name: "Jane Smith", category: "Resident" },
  { name: "Michael Johnson", category: "Tenant" },
  { name: "Emily Davis", category: "Resident" },
  { name: "Christopher Martinez", category: "Tenant" },
  { name: "Linda Clark", category: "Resident" },
  { name: "Laura Thomas", category: "Resident" },
]);
...
async function onAssign() {
  const findCard = accessCards.value.find(
    (find: any) => find.cardNo == selectedCardItem.value.cardNo,
  );
  if (findCard) {
    findCard.accessBy = selectedUser.value.name;
    findCard.category = selectedUser.value.category;
    findCard.dateAssigned = "02/01/2025";
  }

  assignDialog.value = false;
  setSnackbar({
    text: `Successfully assigned ${selectedUser.value.name} to access card`,
    modal: true,
    type: "success",
  });
}
