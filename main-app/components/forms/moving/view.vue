...
onMounted(async () => {
  try {
    ...
    item.value.approvedBy = userFullName.value;
    item.value.chequeDate = new Date() as unknown as any;
...
const onDownloadPdf = async (item: TMoving) => {
  ...
  try {
    ...
    currentUrl = `/forms/${id}/download/moving?site=${mySite.value}`;
