        ...
        <v-form v-model="isValid" @submit.prevent="onUpdateItem()">
          ...
          <div class="px-10">
                ...
                <v-container>
                  <v-row class="text-h6" align="start" justify="space-between">
                    <v-col cols="auto" class="pa-0">
                      <v-icon
                        :icon="
                          item.isAgreeToAboveCondition
                            ? 'mdi-check-circle'
                            : 'mdi-close-circle'
                        "
                        :color="item.isAgreeToAboveCondition ? 'green' : 'red'"
                      />
                    </v-col>
            ...
            <div>
              ...
              <p class="mt-6 mb-5">
                I/We {{ item.recipient }} hereby acknowledge receipt of bicycle
                tag
              </p>
...
const onUpdateItem = async () => {
  ...
  const updatePayload = {
    tagNumber: item.value.tagNumber,
    approvedBy: currentUser.value._id,
