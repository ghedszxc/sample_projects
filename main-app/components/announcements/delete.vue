<template>
  <v-dialog v-model="deleteAnnouncementDialog" max-width="500">
    <v-card
      :loading="isProgress"
      :style="{ border: `${theme == 'light' && 'thin solid #E8E8E8'}` }"
      :class="`${theme == 'light' && 'bg-white'} h-100 ${$vuetify.display.mdAndUp}`"
    >
...
async function deleteAnnouncement() {
  ...
  try {
    await deleteAnnouncementById(announcement.value._id as string);
    await updateFilter();
    onCloseDialog();
    setSnackbar({
      text: "Bulletin deleted successfully",
      modal: true,
      type: "success",
    });
  } catch (err) {
