<template>
            ...
            <v-row no-gutters>
              ...
              <v-row no-gutters class="pa-0 ma-0">
                <v-col cols="12" md="4" lg="4" xl="4" class="pa-4 ma-0">
                  <h3 class="titleDesign pb-6 font-weight-medium">
                  ...
                  <v-col cols="12" class="mt-1" v-if="facility.featuredImage">
                    <v-chip-group column multiple>
                      ...
                      >
                        <v-chip
                          closable
                          @click:close="removeFile(facility.featuredImage._id)"
                  ...
                  <v-chip-group column multiple>
                    <template v-for="file in facility?.gallery" :key="file._id">
                      <NuxtLink
                ...
                <v-col cols="12" md="4" lg="4" xl="4" class="pa-4 ma-0">
                  ...
                  <h3 class="titleDesign mt-10">Facility Availability</h3>
                  <v-card class="ma-2">
                    <v-col
                      cols="12"
                ...
                <v-col cols="12" md="4" lg="4" xl="4" class="pa-4 ma-0">
                  ...
                  <v-col cols="12">
                    <tiptap-editor
                      ...
                      :hideBubble="true"
                      :initialContent="facility.description"
                      @content-updated="handleDescriptionUpdated"
                    />
                  </v-col>

                  <h3 class="titleDesign">Facility Rules and Policies</h3>

                  <v-col cols="12">
                    <tiptap-editor
                      :removeDefaultWrapper="true"
                      :hideBubble="true"
                      :initialContent="facility.rulesAndPolicies"
                      @content-updated="handleRulesUpdated"
                    />
                  </v-col>
                  <!-- <v-col cols="12">
                    <v-textarea
                      v-model="facility.rulesAndPolicies"
  ...
  >
    <v-card
      ...
      :loading="isAffectedBookingsLoading"
    >
      <template v-slot:loader="{ isActive }">
        <v-progress-linear
          :active="isActive"
      ...
      <v-card-text>
        <v-col cols="12">
          <v-row no-gutters>
            <div v-if="affectedBookings?.items.length">
              {{
                `Below is the list of all the affected ${affectedBookingsTab} bookings that needs to be
            ${affectedBookingsTab !== "pending" ? "CANCELLED" : "REJECTED"} in order to update this facility`
              }}
              :
            </div>
            <v-col cols="12">
              <v-data-iterator
                :items="affectedBookings?.items"
                :items-per-page="selectedPageRange"
              >
                <!-- header -->
                <template v-slot:header>
                  <v-list
                    :class="`pa-0 pt-${$vuetify.display.mdAndUp ? '5' : '0'}`"
                    density="compact"
                  ...
                  >
                    ...
                    >
                      <v-list-item-title>
                        <v-row no-gutters>
                          <v-col cols="12" md="4" class="text-capitalize text">
                            <v-row
                              no-gutters
                              class="fill-height"
                              align-content="center"
                          ...
                          <v-col cols="12" md="3" class="text-capitalize text">
                            <v-row
                              no-gutters
                              class="fill-height"
                              align-content="center"
                          ...
                          <v-col cols="12" md="1" class="text-capitalize text">
                            <v-row
                              no-gutters
                              class="fill-height"
                              align-content="center"
            ...
            <!-- pagination -->
            <v-col v-if="affectedBookings?.items.length" cols="12" class="pa-4">
              <v-row no-gutters>
                <v-col cols="3">
                  <v-select
                    :items="pageRanges"
                ...
                <v-spacer></v-spacer>
                <v-col cols="4">
                  <span>{{ affectedBookings.pageRange }}</span>
                  <v-btn
                    density="compact"
                    variant="text"
                    icon
        ...
        <!-- remarks -->
        <v-col v-if="affectedBookings?.items.length" cols="12" md="6">
          <v-textarea
            v-model="remarks"
            label="Remarks"
            placeholder="Remarks"
        ...
        <!-- cancel / close -->
        <v-col v-if="affectedBookings?.items.length" cols="12">
          <v-row no-gutters>
            <v-btn
              flat
              color="error"
            ...
            >
              {{
                `${affectedBookings?.items.filter((i: any) => i.status == "approved").length ? "Cancel" : "Reject"}`
              }}
              All
            </v-btn>
            <v-btn
...
const { requiredInput, getInitial, materialColors, standardFormatDate } =
  useUtils();
...
const { updateStatus } = useFacilityBookings();
...
const isAffectedBookingsLoading = ref<boolean>(false);
...
function applyTimeSlotsToSelectedDays(Days: string[]) {
  for (const day of Days) {
    console.log(`Facility ${day}`, facility.value[day]);
...
watch(
  ...
  async (newValue: string) => {
    ...
    affectedBookings.value = await getAffectedBookedSlot({
      limit: selectedPageRange.value,
      facility: facility.value._id,
      site: mySite.value,
      timeSlots: facility.value.timeSlots,
      status: newValue,
    });
...
async function saveData() {
  loading.value = true;
  try {
    const result: any = await editFacilityById(facility.value);
    ...
    if (result.isWithBookedSlot) {
      ...
      dialog.value = true;
      totalPages.value = affectedBookings.value.pages;
      timeSlotsData.value = result.timeSlots;
    } else {
      setSnackbar({
...
async function cancel() {
  loading.value = true;
  try {
    const _approvedIds = affectedBookings.value.items
    ...
    const _pendingIds = affectedBookings.value.items
...
function closeDialog() {
  ...
  selectedDaysToApplyTheSelectedTimeSlot.value = [];
}
...
function saveFeaturedImage() {
  file.value = featuredImage.value.map((file) => ({
    name: file.name,
    data: file,
    progress: 0,
