...
const { deleteUserById, getUserByIdLocal } = useUser();
...
async function submit() {
  try {
    await deleteUserById({ id: id.value });
    setSnackbar({ text: "User Deleted", modal: true, type: "success" });
    router.push({ name: "team" });
  } catch (error: any) {
    const errorMessage = error.response
      ? error.response.data.message
      : "Unknown error occurred";
    setSnackbar({ text: errorMessage, modal: true, type: "error" });
  }
}
interface User {
  givenName?: string;
