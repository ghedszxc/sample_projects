  ...
  <v-row no-gutters class="pa-8">
    <v-col cols="12">
      <base-table
        ...
        :pages="pages"
        :isLoading="isLoading"
        noDataMsg="You don't have any feedback yet."
        noDataImg="/empty.svg"
          ...
          <v-row no-gutters align="center">
            ...
            <v-col cols="12" md="12" lg="8" class="mt-md-4 pl-md-1 mt-lg-0">
              <v-row class="d-flex align-center">
                ...
                <v-col
                  v-if="$ability.can('create', 'create-feedbacks')"
                  cols="12"
                  md="3"
                >
                  <!-- <v-btn
                  ...
                  >
                    <v-icon>mdi-plus</v-icon>Feedback
                  </v-btn> -->
                  <v-btn
                    color="blue-darken-3"
                    block
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            {{ getInitial(item.raw.createdByName || "") }}
                          </v-avatar>
                          <span class="subject-text">
                            {{ item.raw.createdByName }}</span
                          >
                        </span>
                      </v-row>
                    </v-col>
                        ...
                        <!-- <v-menu>
                          <template #activator="{ props }">
                            <v-btn v-bind="props" icon flat size="x-small">
                              <v-icon
                                density="compact"
                                v-bind="props"
                                class="rounded-xl pa-4 border-md"
                              >
                          ...
                          <v-card>
                            <v-list density="compact" class="pa-0">
                              <v-list-item
                                v-if="
                                  $ability.can('delete', 'delete-feedbacks')
                                "
                                :to="{
                                  name: 'feedbacks-id-delete',
                              ...
                              <v-list-item
                                v-if="
                                  item.raw.status &&
                                  (/^(pending|on-going|on going|to-do|todo)$/i.test(
                                    item.raw.status,
                                  ) ||
                                    $ability.can('update', 'edit-feedbacks'))
                                "
                                :to="{
                                  name: 'feedbacks-id-assignee',
                                  params: { id: item.raw._id },
                                  query: { site: mySite },
                                }"
                              >
...
async function changePath(data: TFeedback) {
  useRouter().push({
    name: "feedbacks-id",
    params: {
      id: data._id,
...
const handleClickFeedback = () => {
  ...
  if (currentUser.value.type == "site") {
    ...
    setSnackbar({
      text: `Only site personnel are allowed to create feedback.`,
      modal: true,
      type: "error",
    });
  }
