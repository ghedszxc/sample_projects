...
const { isCheckInImageLoading, getVisitorCheckInImage } = useVisitor();
const {
...
const checkInImageId = computed(() =>
  useRoute().params.checkInImageId
    ? String(useRoute().params.checkInImageId)
    : "",
);

function printImage() {
  const divContent = document.getElementById("check_in_image")?.innerHTML || "";
...
onMounted(async () => {
  isCheckInImageLoading.value = true;

  const result: any = await getVisitorCheckInImage(checkInImageId.value);
  imageBase64String.value = `data:image/jpeg;base64,${result.data}`;
  isCheckInImageLoading.value = false;
});
