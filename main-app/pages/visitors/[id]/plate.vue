...
const { visitor, getVisitorById, setVisitor, isVisitorValid, updateVisitor } = useVisitor();
...
async function submit() {
	try {
		const result = await updateVisitor({
			...
			plateNumber: visitor.value.plateNumber,
		});
		setSnackbar({ text: "Successfully update visitor Vehicle Number", modal: true, type: "success" });
		goBack();
	} catch (error) {
