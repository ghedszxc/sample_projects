      ...
      >
        <v-form v-model="isVisitorValid" @submit.prevent="submit()">
          <v-row no-gutters class="pa-8" justify="center">
            ...
            <v-col>
              <span class="font-weight-bold"> Vehicle Number </span>
              <span class="text-red mx-1 mb-2">*</span>
            </v-col>
            <v-col cols="12" class="mb-4">
              <v-text-field
                v-model="visitor.plate_number"
                density="comfortable"
                :rules="[requiredInput]"
...
const { visitor, updateVisitor, setVisitor, isVisitorValid } = useVisitor();
...
async function submit() {
  try {
    const result = await updateVisitor(visitor.value);
    setSnackbar({ text: result, modal: true });
    goBack();
  } catch (error) {
    setSnackbar({ text: error as string, modal: true });
  }
}
</script>
