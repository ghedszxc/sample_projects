<template>
  <v-row no-gutters justify="center" class="py-md-8">
    <v-col cols="12" md="10" class="font-weight-light mt-md-4">
      <v-card elevation="0" style="border-width: thin" min-height="368px">
...
function nextStep2() {
  ...
  const incidentEnd = incidentInformation.incidentTypeAndTime.incidentEnd;
  if (incidentEnd !== "Pending" && incidentEnd !== "N/A") {
    timeValidation(incidentEnd, "incidentEnd");
  }
  timeValidation(incidentInformation.submissionForm.time, "responseTime");
  timeValidation(
    incidentInformation.submissionForm.timeOfResponse,
  ...
  if (incidentEnd !== "Pending" && incidentEnd !== "N/A") {
    isValidValueTime.push(timeValueValidation(incidentEnd));
  }
  isValidValueTime.push(
    timeValueValidation(incidentInformation.submissionForm.time),
  );
...
async function confirmSign(user?: TUser) {
  if (user && typeof user === "object") {
    ...
    try {
      ...
      useRouter().replace({ name: "incidents", query: { site: mySite.value } });
      setSnackbar({
